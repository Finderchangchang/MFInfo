...
/**
 * Created by bing.ma on 2017/7/17.
 */
...
//        Log.i(test, 新尺寸: +  targetwidth + * +targetheight);
    ...
    public static void compressLoad(final ImageView imageView, final String photoFilePath, final int width, final int hight) {
        imageView.setTag(photoFilePath);
        Observable.just(photoFilePath) // 输入类型 String
                .map(new Func1<String, Bitmap>() {
                    @Override
                    public Bitmap call(String s) {
                        return BitmapCompressor.decodeSampledBitmapFromFile(s, width, hight);
                    }
                })
                ...
                .subscribe(new Action1<Bitmap>() {
                    @Override
                    public void call(Bitmap btm) { // 参数类型 Bitmap
                        String tag = (String) imageView.getTag();
                        if (tag.equals(photoFilePath)) {
