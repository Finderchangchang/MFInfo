...
public class CommonViewHolder {
     ...
     * @return
     */
    public CommonViewHolder setRBChecked(int viewId, boolean text) {
    ...
    //多选框选择
    public CommonViewHolder setCheck(int viewId, boolean ischeck) {
    ...
    public CommonViewHolder setGImage(int vid, String url) {
        return this.setGImage(vid, url, 0);
    }

    //正常加载图片
    public CommonViewHolder setGImage(int vid, String url, int height) {
        ...
        final String tag = (String) img.getTag(R.id.imageloader_uri);
        if (!url.equals(tag)) {
            img.setTag(R.id.imageloader_uri, url);
            if (mContext != null && !TextUtils.isEmpty(url)) {
                String uir = new urls().getTotal() + "upload/" + url;
                Glide.with(mContext).load(new urls().getTotal() + "upload/" + url).error(R.mipmap.fen_bg).into(img);
    ...
    public CommonViewHolder setGlideImage(int vid, String url) {
        ImageView img = getView(vid);
        if (mContext != null) {
            GlideImgManager.glideLoader(mContext, url, R.mipmap.fen_bg, R.mipmap.fen_bg, img, 0);
        }
        return this;
    }
    ...
    public CommonViewHolder setRoundImage(int vid, String url) {
        ImageView img = getView(vid);
        GlideImgManager.glideLoader(mContext, url, R.mipmap.fen_bg, R.mipmap.fen_bg, img, 1);
        return this;
    }
