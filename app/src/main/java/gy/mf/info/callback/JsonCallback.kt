...
import gy.mf.info.callback.Convert
import gy.mf.info.callback.SimpleResponse
...
abstract class JsonCallback<T> : AbsCallback<T>() {
    ...
    override fun onBefore(request: BaseRequest<*>?) {
        ...
        request!!.params("token", App.token)
    }
     ...
     * .tag(this)//
     * .execute(new DialogCallback<TotalModel></TotalModel><ServerModel>>(this) {
     * @Override
     * *          public void onSuccess(TotalModel<ServerModel> responseData, Call call, Response response) {
    ...
    override fun convertSuccess(response: Response): T {
        ...
        //com.lzy.demo.model.TotalModel<com.lzy.demo.model.ServerModel>
        ...
        //class com.lzy.demo.model.TotalModel
        ...
        if (typeArgument === Void::class.java) {
            //无数据类型,表示没有data数据的情况（以  new DialogCallback<TotalModel<Void>>(this)  以这种形式传递的泛型)
        ...
        } else if (rawType === TotalModel::class.java) {
            //有数据类型，表示有data
            val TotalModel = Convert.fromJson<TotalModel<*>>(jsonReader, type)
            response.close()
            val code = TotalModel.code
            //一般来说服务器会和客户端约定一个数表示成功，其余的表示失败，这里根据实际情况修改
            if (code == 200) {
                return TotalModel as T
            } else if (code == 104) {
                ...
                throw IllegalStateException(TotalModel.msg)
            }
        } else {
            response.close()
